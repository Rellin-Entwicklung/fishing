...
class ImageProcessorGUI:
    ...
    def _create_widgets(self):
        ...
        # Eingangsverzeichnis
        input_frame = tk.LabelFrame(self.root, text="Eingangsverzeichnis", padx=10, pady=10, bg="white")
        input_frame.pack(fill="x", padx=20, pady=5)
        
        self.input_label = tk.Label(input_frame, text="Kein Verzeichnis ausgewählt", fg="gray", bg="white")
        ...
        # Ausgangsverzeichnis
        output_frame = tk.LabelFrame(self.root, text="Ausgangsverzeichnis", padx=10, pady=10, bg="white")
        output_frame.pack(fill="x", padx=20, pady=5)
        
        self.output_label = tk.Label(output_frame, text="Kein Verzeichnis ausgewählt", fg="gray", bg="white")
        ...
        # Einstellungen
        settings_frame = tk.LabelFrame(self.root, text="Einstellungen", padx=10, pady=10, bg="white")
        settings_frame.pack(fill="x", padx=20, pady=5)
        
        # Größe
        size_frame = tk.Frame(settings_frame, bg="white")
        size_frame.pack(fill="x", pady=5)
        tk.Label(size_frame, text="Ausgabegröße (Pixel):", bg="white").pack(side="left")
        ...
        tk.Label(quality_frame, text="WebP-Qualität (0-100):", bg="white").pack(side="left")
        ...
        # Drehung
        rotation_frame = tk.Frame(settings_frame, bg="white")
        rotation_frame.pack(fill="x", pady=5)
        tk.Label(rotation_frame, text="Drehung:", bg="white").pack(side="left")
        self.rotation_var = tk.StringVar(value="90°")
        rotation_combo = ttk.Combobox(
            rotation_frame,
            textvariable=self.rotation_var,
            values=["Keine", "90°", "180°", "270°"],
            state="readonly",
            width=8
        )
        ...
        opt_frame = tk.LabelFrame(settings_frame, text="Bildoptimierung (optional)", padx=5, pady=5, bg="white")
        opt_frame.pack(fill="x", pady=5)
        self.opt_enabled_var = tk.BooleanVar(value=False)
        opt_cb = tk.Checkbutton(
            opt_frame,
            text="Optimierung aktivieren (Helligkeit + Sättigung)",
            variable=self.opt_enabled_var,
            bg="white"
        )
        opt_cb.pack(anchor="w")
        opt_row = tk.Frame(opt_frame, bg="white")
        opt_row.pack(fill="x", pady=3)
        tk.Label(opt_row, text="Helligkeit (+%):", bg="white", width=14).pack(side="left")
        self.opt_brightness_var = tk.IntVar(value=15)
        brightness_spin = tk.Spinbox(opt_row, from_=0, to=100, increment=5, textvariable=self.opt_brightness_var, width=6)
        brightness_spin.pack(side="left", padx=(0, 15))
        tk.Label(opt_row, text="Sättigung (+%):", bg="white", width=14).pack(side="left")
        self.opt_saturation_var = tk.IntVar(value=15)
        saturation_spin = tk.Spinbox(opt_row, from_=0, to=100, increment=5, textvariable=self.opt_saturation_var, width=6)
        saturation_spin.pack(side="left")

        # Fortschrittsanzeige
        progress_frame = tk.LabelFrame(self.root, text="Fortschritt", padx=10, pady=10, bg="white")
        ...
        for idx, input_file in enumerate(image_files, 1):
            try:
                ...
                self._process_single_image(
                    ...
                    opt_enabled=self.opt_enabled_var.get(),
                    opt_brightness=self.opt_brightness_var.get(),
                    opt_saturation=self.opt_saturation_var.get(),
                )
    ...
    def _process_single_image(
        ...
        # 2. Begrenzungsrahmen des freigestellten Objekts ermitteln
        bbox = img.getbbox()
        if bbox is None:
            raise ValueError("Kein sichtbares Objekt nach Hintergrundentfernung gefunden.")

        # 3. Objekt eng zuschneiden
        obj = img.crop(bbox)
        w, h = obj.size
        side = max(w, h)

        # 4. Objekt exakt mittig auf quadratisches Canvas setzen
        canvas = Image.new("RGBA", (side, side), (255, 255, 255, 255))
        paste_x = (side - w) // 2
        paste_y = (side - h) // 2
        canvas.paste(obj, (paste_x, paste_y), obj)

        # 5. Optional: Drehen (Keine, 90°, 180°, 270°)
        _rotation_map = {"90°": Image.ROTATE_90, "180°": Image.ROTATE_180, "270°": Image.ROTATE_270}
        rot = _rotation_map.get(rotation)
        if rot is not None:
            canvas = canvas.transpose(rot)

        # 6. Auf Zielgroesse skalieren (quadratisch zu quadratisch, kein Versatz)
        canvas = canvas.resize((size, size), Image.Resampling.LANCZOS)

        # 7. Alpha auf weissen Hintergrund flach legen
        result = Image.new("RGB", (size, size), (255, 255, 255))
        result.paste(canvas, mask=canvas.split()[3])

        # 8. Optional: Bildoptimierung (Helligkeit + Sättigung)
        if opt_enabled:
            brightness_factor = 1.0 + (opt_brightness / 100.0)
            saturation_factor = 1.0 + (opt_saturation / 100.0)
